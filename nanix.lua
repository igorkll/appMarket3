...
for i = 1, inputsCount do
    ...
    b = main.createButton(1, i + 2, listsStart - 2, 1, "", function(state, oldState, button)
        if button == 1 then
            ...
            local input = controlScene.createInputbox(2, 9, 18, 1, "новая заметка", function(read)
                cfg.notes[i] = read
                saveCfg()
                updateText()
                updateLabels()
...
list.addStr("иначе связь может прерваться")
list.addStr("если это произошло")
list.addStr("нажмите на кнопку refresh")
list.addStr(string.rep("-", list.sizeX))
